...
class ChatConsumer(AsyncWebsocketConsumer):
	...
	async def receive(self, text_data):
		...
		if use == 'chat_message':
			...
			logging.info(
				f"Empfangen: Nachricht='{message}', Benutzer='{username}', Raum='{room_name}', Raum_ID'{room_id}'"
			)
			...
			await self.channel_layer.group_send(
				str(
					room_id
				),  # Die Raum-ID als Gruppenname, der zur Identifikation der Gruppe verwendet wird
				{
			...
			logging.info(
				f"Empfangen: Nachricht='{message}', Raum='{room_name}', Raum_ID'{room_id}'"
			)
